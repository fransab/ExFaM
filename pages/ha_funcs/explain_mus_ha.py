...
from .lef import get_var

import os 

def is_alloc(node):
    return node.startswith("ALLOC") and not "_" in node
def is_am(node):
    return node.startswith("ALLOC") and not is_alloc(node)
def is_lef(node):
    return node.startswith("LEF")
...
class SAT_Imp_Graph:
    ...
    def rec_propagate(self,frontier=["T"], old_active=None, extending_vars=True, depth=0):
        ...
        for node in frontier:
            ...
            if extending_vars:
                ...
                if node == "B":
                    ...
                    if self.textual: ex=text_expl(node, self.x, self.p); self.e.append(ex)
        ...
        if self.textual:
            ...
            self.e.append( text_expl(new_active, self.x, self.p) )
...
def text_expl(nodes, x, prefp = None):
    """ 
    ...
    for node in nodes:
        if type(node) == int:
            i,j = get_var(x, node) 
            text += f"Assume agent {i} receives object {j}."
        elif node == "B":
            text += "Contradiction."
        elif is_alloc(node):
            i = eval(node[5:])[0]
